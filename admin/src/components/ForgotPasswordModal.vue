...
<template>
    <div v-if="modelValue.show.value"
        ...
        <div class="bg-white rounded-2xl shadow-xl p-8 w-full max-w-sm mx-4 relative" @click.stop>
            ...
            <h2 class="text-2xl font-semibold text-slate-800 mb-1">{{ $t('forgotPasswordModal.title') }}</h2>
            <p class="text-sm text-slate-500 mb-6">
                {{ $t('forgotPasswordModal.subtitle') }}
            </p>

            <div v-if="modelValue.done.value" class="text-center py-4">
                <p class="text-green-500 font-semibold text-base mb-2">{{ $t('forgotPasswordModal.successTitle') }}</p>
                <p class="text-sm text-slate-600 mb-5">{{ $t('forgotPasswordModal.successMsg') }}</p>
                <button @click="modelValue.close()"
                    class="w-full h-11 bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg font-semibold transition-colors cursor-pointer">
                    {{ $t('forgotPasswordModal.backToLogin') }}
                </button>
            </div>

            <form v-else @submit.prevent="modelValue.submit()" class="flex flex-col gap-4">
                <div class="relative">
                    <input v-model="modelValue.email.value" type="email"
                        :placeholder="$t('forgotPasswordModal.emailLabel')" required
                ...
                <div class="relative">
                    <input v-model="modelValue.newPassword.value"
                        :type="modelValue.showNewPassword.value ? 'text' : 'password'"
                        :placeholder="$t('forgotPasswordModal.newPassword')" required
                        class="w-full py-3 pl-5 pr-12 bg-slate-100 rounded-lg border-none outline-none text-base text-slate-700 placeholder:text-slate-400" />
                    <button type="button" @click="modelValue.showNewPassword.value = !modelValue.showNewPassword.value"
                        class="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 transition-colors cursor-pointer"
                        tabindex="-1">
                        <svg v-if="modelValue.showNewPassword.value" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5"
                ...
                <div class="relative">
                    <input v-model="modelValue.confirmPassword.value" type="password"
                        :placeholder="$t('forgotPasswordModal.confirmPassword')" required
                        class="w-full py-3 pl-5 pr-12 bg-slate-100 rounded-lg border-none outline-none text-base text-slate-700 placeholder:text-slate-400" />
                ...
                <button type="submit" :disabled="modelValue.loading.value"
                    ...
                    {{ modelValue.loading.value ? $t('forgotPasswordModal.updating') :
                        $t('forgotPasswordModal.updateBtn') }}
                </button>
            </form>
        </div>
    </div>
