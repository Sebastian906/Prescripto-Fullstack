import {
  ANNUAL_CSV_COLUMNS,
  annualReportToCsv,
} from '../reports/utils/annual-report-csv.util';
import type { AnnualReportRow } from '../reports/reports.service';
  ...
  it('serializa 2 filas con escapado RFC 4180', () => {
    const csv = annualReportToCsv([
      row({
        ...
        earnings: 1500,
        uniquePatients: 9,
        cumulativeEarnings: 1500,
      }),
      row({
        month: 2,
        monthLabel: 'Feb "alta", norte',
        totalAppointments: 5,
        completedAppointments: 5,
        cancelledAppointments: 0,
        earnings: 800,
        uniquePatients: 5,
        cumulativeEarnings: 2300,
      }),
    ]);
