import { getJwtSecrets, isPreviousAccepted } from '../shared/utils/jwt-secrets.util';

const cfg = (m: Record<string, string>) => ({ get: (k: string, fb?: any) => m[k] ?? fb });
...
describe('jwt-secrets.util', () => {
    it('prefers CURRENT, falls back to legacy JWT_SECRET', () => {
        ...
        expect(getJwtSecrets(cfg({ JWT_SECRET: 'old', JWT_SECRET_CURRENT: 'new' })).current).toBe('new');
    });
    it('accepts previous before deadline, rejects after', () => {
        const s = getJwtSecrets(cfg({
            JWT_SECRET_CURRENT: 'new', JWT_SECRET_PREVIOUS: 'old',
