...
export class PasswordResetService {
  ...
  private async sendResetEmail(
    ...
    const adminUrl =
      this.configService.get<string>('VITE_ADMIN_URL') ?? frontendUrl;
  ...
  async resetPassword(
    ...
    if (record.role === 'user') {
      ...
      await this.doctorModel.findByIdAndUpdate(record.userId, {
        password: hashedPassword,
      });
  ...
  async directReset(
    ...
    if (role === 'doctor') {
      ...
      const hashedPassword = await bcrypt.hash(newPassword, 10);
      await this.doctorModel.findByIdAndUpdate(String(doctor._id), {
        password: hashedPassword,
      });
