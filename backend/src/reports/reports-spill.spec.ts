...
function makeStatsModel(state: { inline: string[] | null; uniquePatients: number; updateCalls: Array<{ filter: unknown; update: unknown }> }) {
    return {
...
describe('ReportsService spill (cap 5000)', () => {
    async function build(state: {
        ...
        const spillModel = {
            ...
            countDocuments: jest
                .fn()
                .mockImplementation(() => ({ exec: () => Promise.resolve(state.spill.size) })),
        };
    ...
    it('paciente nuevo con hueco inline usa $addToSet + $inc uniquePatients', async () => {
        ...
        const incUpdate = (statsModel.updateOne as jest.Mock).mock.calls.find(
    ...
    it('paciente duplicado inline no toca spill ni incrementa uniquePatients', async () => {
        ...
        expect(spillModel.create).not.toHaveBeenCalled();
        const withAddToSet = (statsModel.updateOne as jest.Mock).mock.calls.filter(
            ([, u]) => (u as { $addToSet?: unknown }).$addToSet,
        );
    ...
    it('paciente ya en spill solo incrementa contadores', async () => {
        ...
        expect(
            (spillModel.create as jest.Mock).mock.calls.filter(
                ([d]) => d.patientId === 'px',
            ).length,
        ).toBe(0);
    ...
    it('duplicado concurrente en spill (E11000) no duplica uniquePatients', async () => {
        ...
        (spillModel.exists as jest.Mock).mockImplementationOnce(() => ({
            exec: () => Promise.resolve(null),
        }));
        await service.onAppointmentBooked('doc1', 'race', 100, new Date(2025, 4, 10));
        expect(spillModel.create).toHaveBeenCalled();
        const incUnique = (statsModel.updateOne as jest.Mock).mock.calls.filter(
            ([, u]) => JSON.stringify(u).includes('uniquePatients'),
        );
        ...
        const withAdd = (statsModel.updateOne as jest.Mock).mock.calls.filter(
            ([, u]) => (u as { $addToSet?: unknown }).$addToSet,
        );
    ...
    it('6000 pacientes sintéticos: inline queda en 5000 y spill recibe 1000', async () => {
        const state = {
            inline: [] as string[],
            uniquePatients: 0,
    ...
    it('onAppointmentCompleted/Cancelled no tocan arrays de pacientes', async () => {
        const { service, statsModel, spillModel } = await build({
            inline: null as unknown as string[],
            uniquePatients: 0,
            spill: new Set(),
        });
        // findOne devuelve null → path upsert solo si hay patientId; aquí no hay.
        await service.onAppointmentCompleted('doc1', 'u1', 50, new Date(2025, 0, 5));
    ...
    it('GLOBAL_DOC_ID se actualiza junto al doctor', async () => {
        ...
        expect((statsModel.updateOne as jest.Mock).mock.calls.length).toBeGreaterThanOrEqual(2);
    ...
    it('mes/año se extraen correctamente (junio=6)', async () => {
        ...
        const filters = (statsModel.updateOne as jest.Mock).mock.calls.map(
